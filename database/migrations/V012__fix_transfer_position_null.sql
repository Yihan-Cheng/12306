-- 长三角 12306 客票综合销售系统
-- V012: 修复换乘复核过程在未指定座位位置时的 NULL 兼容问题
-- 背景: V010 的 sp_validate_transfer_candidates 在 p_position_code 为空时
--       preferred_available 计算为 NULL，写入 NOT NULL 列导致 1048 错误。
-- 前置迁移: V002 ... V011
-- 目标版本: MySQL 8.4+
...
DROP PROCEDURE IF EXISTS sp_validate_transfer_candidates$$
...
proc: BEGIN
    ...
    SELECT
        ...
        COALESCE(SUM(s.position_code = v_position AND
